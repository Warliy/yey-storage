...
try_to_connect(NautilusDropboxHookserv *hookserv) {
  /* create socket */
//  hookserv->socket = socket(PF_UNIX, SOCK_STREAM, 0); /* philipp commented */
  hookserv->socket = socket(PF_INET, SOCK_STREAM, 0);
  ...
  {
    //struct sockaddr_un addr; /* philipp commented */
   // socklen_t addr_len; /* philipp commented */
    
    /* intialize address structure */
    /*addr.sun_family = AF_UNIX;
    g_snprintf(addr.sun_path,
	       sizeof(addr.sun_path),                // philipp commented
	       "%s/.clonebox/iface_socket",
	       g_get_home_dir());
