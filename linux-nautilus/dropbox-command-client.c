...
//      if (0 > (sock = socket(PF_UNIX, SOCK_STREAM, 0))) { /* philipp CHANGED */
...
//      if (connect(sock, (struct sockaddr *) &addr, addr_len) < 0) { /* philipp CHANGED */
...
//	  if (connect(sock, (struct sockaddr *) &addr, addr_len) < 0) { /* philipp CHANGED */
	  if (connect(sock, (struct sockaddr *) &addr, sizeof(addr)) < 0) {
	    	    debug("couldn't connect to command server after 1 second"); /* philipp comment removed */
	    break;
	  }
	}
	/* errno != EINPROGRESS */
